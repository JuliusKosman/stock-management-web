...
export default function AddStockOutModal({ onClose, onSave, products }) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      console.error("Failed to save stock-out:", err);
    ...
    <>
      <div style={backdropStyle}>
        <div className="modal-content bg-white p-4 rounded shadow" style={{ maxWidth: "500px", width: "100%" }}>
          <h5 className="mb-3 text-danger">Add Stock Out</h5>
          {error && <div className="alert alert-danger">{error}</div>}
          <select className="form-select mb-2" value={selectedID} onChange={(e) => setSelectedID(e.target.value)}>
          ...
          <div className="d-flex justify-content-end gap-2">
            ...
            <button className="btn btn-danger" onClick={handleSave}>Add Stock Out</button>
          </div>
        </div>
      </div>
      ...
      {showPopup && (
        <div style={{
          ...
          <div style={{
            ...
            <p>Stock-out entry saved successfully.</p>
          </div>
        </div>
      )}
    </>
