...
import { FaDownload, FaPlusCircle, FaSearch, FaSignOutAlt } from "react-icons/fa";
  ...
  return (
    <div className="container py-4">
      <h3 className="mb-4 d-flex align-items-center gap-2">
        <FaSignOutAlt /> Stock Out
      </h3>

      {/* Toolbar */}
      <div className="mb-3 d-flex flex-wrap align-items-center gap-2">
        <button className="btn btn-danger" onClick={() => setShowModal(true)}>
          <FaPlusCircle /> Add Stock Out
        </button>
      ...
      {showModal && (
        <AddStockOutModal
          products={products}
          onSave={() => { setShowModal(false); handleAddStockOut(); }}
          onClose={() => setShowModal(false)}
        />
      )}
      {showExport && (
        <ExportModal
          title="Export Stock Out"
          data={filtered}
      ...
      <div className="card shadow-sm">
        <div className="card-body table-responsive">
          {filtered.length === 0 ? (
            <div className="alert alert-info mb-0">No stock-out records found.</div>
          ) : (
            <>
